...
{
  "analise_interna": "Raciocínio detalhado sobre o mapeamento realizado",
...
async def processar_registro(doc, coll_name, pbar):
    async with semaphore:
        destino_coll = db[f"eval_final_{coll_name}_llama"]
        ...
        try:
            ...
            await db["logs_erro_llama"].insert_one({
                "original_id": doc["_id"], 
                "coll": coll_name,
                "erro": str(e)
            })
...
async def main():
    for coll_name in COLLECTIONS:
        ...
        print(f"\n Iniciando Big Run LOCAL (Llama): {coll_name} ({total_docs} docs)")
        
        with tqdm(total=total_docs, desc=f"Auditando {coll_name}") as pbar:
            tarefas = [] 
...
if __name__ == "__main__":
    try:
        ...
        print("\n\nProcesso interrompido pelo usuário. Checkpoint salvo.")